...
> **Статус:** каркас (scaffold) всех 16 сервисов готов и проверен E2E
> (Postgres + NATS + CRUD + публикация событий). Семантика функций
> (генерация заголовков, рекурренс, RAG, саги) помечена `📋` в `FEATURES.md`
> и реализуется поверх каркаса.
...
`contracts/asyncapi/events.yaml` → `x-implemented-wire-events`. Остальные события
в каталоге (`title_generated`, `text_extracted`, `reminder`, …) — `📋` запланированы
(намерение из `FEATURES.md`/`SAGA.md`), каркасом пока не шлются.
