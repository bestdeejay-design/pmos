...
const tableCols = new Set<string>(["id", "kind", "model", "promptChars", "createdAt"]);
...
export const ai_gatewayRoutes: FastifyPluginAsync = async (app) => {
  ...
  typed.post("/restore-punctuation", async (_req, reply) => {
    // TODO(semantics): POST /restore-punctuation — non-CRUD endpoint, not in the baseline
  ...
  typed.post("/dictate", async (_req, reply) => {
    // TODO(semantics): POST /dictate — non-CRUD endpoint, not in the baseline
    // reference pattern. Implement domain logic or remove from contract.
    return reply.code(501).send({ code: "NOT_IMPLEMENTED", message: "endpoint planned (see AGENT.md §4 backlog)" });
  });
};
