...
async function main() {
  ...
  logger.info({ service: "projects", port: PORT }, "service started");

  // Event bus bootstrap (ADR-003 / ADR-007 §3).
  // Subscribers are registered in buildApp() (src/app.ts) — do not re-register here.
  const bus = EventBus.init({ serviceName: "projects", eventVersion: 1 });
  try {
    await bus.connect();
    await bus.ensureStream();
    logger.info({ service: "projects" }, "event bus connected");
  } catch (err) {
