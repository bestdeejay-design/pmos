...
async function main() {
  ...
  logger.info({ service: "settings", port: PORT }, "service started");

  // Event bus bootstrap (ADR-003 / ADR-007 §3)
  const bus = EventBus.init({ serviceName: "settings", eventVersion: 1 });
  try {
    await bus.connect();
    await bus.ensureStream();
    await import("./events/subscribe.js").then((m) => m.registerSubscribers(bus));
    logger.info({ service: "settings" }, "event bus connected");
  } catch (err) {
