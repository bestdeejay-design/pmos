...
const tableCols = new Set<string>(["id", "path", "autoImport", "autoExport", "profileScope", "lastScanAt", "createdAt", "updatedAt"]);
...
export const syncRoutes: FastifyPluginAsync = async (app) => {
  ...
  typed.get("/sync-folders", {
    ...
    const rows = await db.select().from(schema.syncFolders).where(where)
      .orderBy(asc(schema.syncFolders.createdAt)).limit(limit).offset(offset);
    const total = await totalOf(schema.syncFolders, where);
