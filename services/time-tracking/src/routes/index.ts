...
export const timeTrackingRoutes: FastifyPluginAsync = async (app) => {
  ...
  typed.get("/", {
    ...
    const rows = await db.select().from(schema.timesheet).limit(limit).offset(offset);
    const total = await totalOf(schema.timesheet);
  ...
  typed.post("/", {
    ...
    durationSec: Type.Optional(Type.Integer()),
  ...
  typed.patch("/:id", {
    ...
    const [row] = await db.update(schema.timesheet).set({ ...(req.body as any), updatedAt: new Date() })
